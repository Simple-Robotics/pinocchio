...
int main()
{
  ...
  pose_s(0) = 1.0;
  pose_s(1) = 1.0;
  pose_s(2) = cos(M_PI / 4.0);
  pose_s(3) = sin(M_PI / 4.0);

  // Goal configuration
  pose_g(0) = 3.0;
  pose_g(1) = -1.0;
  pose_g(2) = cos(-M_PI / 2.0);
  pose_g(3) = sin(-M_PI / 2.0);
  ...
  // Check that the composition of the starting configuration and the difference vector gives the
  // goal configuration
