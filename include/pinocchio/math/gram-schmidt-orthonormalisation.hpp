...
namespace pinocchio
{
  /// \brief Perform the Gram-Schmidt orthonormalisation on the input/output vector for a given input basis
  ///
  /// \param[in] basis Orthonormal basis
  /// \param[in,out] vec Vector to orthonomarlize wrt the input basis
  ///
  template<typename MatrixType, typename VectorType>
  void orthonormalisation(const Eigen::MatrixBase<MatrixType> & basis, 
