...
namespace pinocchio
{
  namespace container
  {
    ...
    ///
    /// \brief Specialization of an std::vector with an aligned allocator. This specialization might
    /// be used when the type T is or contains some Eigen members.
    ///
    ...
    struct aligned_vector : public std::vector<T, Eigen::aligned_allocator<T>>
    {
      ...
      explicit aligned_vector(const allocator_type & a = allocator_type())
      : vector_base(a)
      {
      }
      ...
      vector_base & base()
      {
        return *static_cast<vector_base *>(this);
      }
      const vector_base & base() const
      {
        return *static_cast<const vector_base *>(this);
      }
