...
namespace pinocchio
{
                             ...
                             const Eigen::MatrixBase<Matrix6or3Like> & J_)
  {
    ...
    for(size_t k = 0; k < colwise_span_indexes.size(); ++k)
    {
      ...
      const int sign =
      colwise_joint1_sparsity[col_id] != colwise_joint2_sparsity[col_id]
          ...
          switch(constraint_model.reference_frame)
          {
            ...
            case LOCAL:
            {
              ...
              else // sign == -1
              {
                ...
                Jcol_out.noalias() = -c1Mc2.rotation() * Jcol_local.linear(); // TODO: simplify computations
              }
              break;
            }
            case LOCAL_WORLD_ALIGNED:
            {
              if(sign == 0)
              {
                Jcol_out.noalias() = (oMc2.translation() - oMc1.translation()).cross(Jcol_motion_in.angular());
              }
              else
              {
                if(sign == 1)
                  Jcol_out.noalias() = Jcol_motion_in.linear() - oMc1.translation().cross(Jcol_motion_in.angular());
                else
                  Jcol_out.noalias() = -Jcol_motion_in.linear() + oMc2.translation().cross(Jcol_motion_in.angular());
              }
              break;
            }
          }
