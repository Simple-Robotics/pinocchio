...
namespace pinocchio
{
  ...
  ///
  /// \brief Computes all the terms required to compute the derivatives of the placement, spatial
  /// velocity
  ...
  ///
  /// \remarks This function is similar to do a forwardKinematics(model,data,q,v) followed by a
  /// computeJointJacobians(model,data,q).
  ///          In addition, it computes the spatial velocity of the joint expressed in the world
  ///          frame (see data.ov).
  ///
  template<
    typename Scalar,
  ...
  ///        You must first call computForwardKinematicsDerivatives before calling this function.
  ///        It is important to notice that a direct outcome (for free) of this algo is v_partial_dq
  ///        and v_partial_dv which is equal to a_partial_da.
  ///
  /// \tparam JointCollection Collection of Joint types.
  ...
  ///
  /// \brief Computes the partial derivatives of the velocity of a point given by its placement
  /// information w.r.t. the joint frame.
  ///        You must first call computForwardKinematicsDerivatives before calling this function.
  ///
  /// \tparam JointCollection Collection of Joint types.
  ...
  ///
  /// \brief Computes the partial derivatives of the classic acceleration of a point given by its
  /// placement information w.r.t. the joint frame.
  ///        You must first call computForwardKinematicsDerivatives before calling this function.
  ///        It is important to notice that a direct outcome (for free) of this algo is
  ///        v_point_partial_dq. v_point_partial_dv is not computed it is equal to
  ///        a_point_partial_da.
  ///
  /// \tparam JointCollection Collection of Joint types.
  /// \tparam Matrix3xOut1 Matrix3x containing the partial derivatives of the spatial velocity with
  ...
  ///
  /// \brief Computes the partial derivaties of the classic acceleration of a point given by its
  /// placement information w.r.t. to the joint frame.
  ///        You must first call computForwardKinematicsDerivatives before calling this function.
  ///        It is important to notice that a direct outcome (for free) of this algo is
  ///        v_point_partial_dq and v_point_partial_dv..
  ///
  /// \tparam JointCollection Collection of Joint types.
  /// \tparam Matrix3xOut1 Matrix3x containing the partial derivatives of the spatial velocity with
  /// respect to the joint configuration vector. \tparam Matrix3xOut2 Matrix3x containing the
  ...
  ///
  /// \brief Computes all the terms required to compute the second order derivatives of the
  /// placement information, also know as the
  ///        kinematic Hessian. This function assumes that the joint Jacobians (a.k.a data.J) has
  ///        been computed first. See \ref computeJointJacobians for such a function.
  ///
  ...
  /// expressed.
  ///
  /// \returns The kinematic Hessian of the joint provided by its joint_id and expressed in the
  /// frame precised by the variable rf.
  ///
  ...
  ///          Please refer to getJointKinematicHessian for a version without dynamic memory
  ///          allocation.
  ///
  template<typename Scalar, int Options, template<typename, int> class JointCollectionTpl>
  Tensor<Scalar, 3, Options> getJointKinematicHessian(
    const ModelTpl<Scalar, Options, JointCollectionTpl> & model,
    const DataTpl<Scalar, Options, JointCollectionTpl> & data,
    const Model::JointIndex joint_id,
    const ReferenceFrame rf)
